...
/*
与SendMessage是类似的，主要区别:
PostMessage消息发出后不等消息函数处理完就执行它的下一语句    (异步)
SendMessage消息发出后阻塞等消息函数处理完执行它的下一语句    (同步)
*/
//向窗口发送 信息--包含错误 状态，通过在窗口访问TthreadLog线程的sentmessage来获得消息列表
void SendStatusMsgToWnd()
{
	CWnd* pRecvWnd = pRecvWnd = ::AfxGetApp()->m_pMainWnd;
	if (pRecvWnd == NULL)
		return;
	HWND hWnd =pRecvWnd->m_hWnd;
	if(hWnd==NULL) //无效窗口句柄
		return;
	
	if (!PostMessage(hWnd, WM_MSG_UPDATE_LOG, LOG_STATUS,NULL))
	{
		;
	}
}
...
void SendErrorMsgToWnd()
{
	...
	if (!PostMessage(hWnd, WM_MSG_UPDATE_LOG, LOG_ERROR, NULL))
	{
		;
	}
}
...
void UnZip(char *src, char *dst)  //解压缩函数
{
	...
	str.assign("bunzip2 -k "); //保留原始压缩数据，使用参数-k
	s1.assign(src);
	str.append(s1);
	//str += " ";
	//str += s2;
...
uint32_t RefreshSharedChannel(LPVOID pParam)
{
	return 0;
	...
	errno_t err = 0;
	char szChar[PATH_LEN] = "";
	TcharToChar(strPath.GetBuffer(), szChar);
	strPath.ReleaseBuffer();
	err=fopen_s(&fp, szChar,"w");	
	if(err!=0)
	{
		fprintf_s(fp, "%d",int( LastTim));
		fclose(fp);
	}

	//while(TRUE && !PROCESSSTPD)
	while(IsPlayBackRunning() || IsRealPlayRunning())
	{		
		// TOD Add your command handler code here
		
		time(&t);
		if(t-LastTim>3*SEC_IN_MIN || t<LastTim)
		{
			char  szChar[PATH_LEN] = "";
			TcharToChar(strPath.GetBuffer(), szChar);
			strPath.ReleaseBuffer();
