...
#define SC_RADAR_DATA_HEADER_LEN	1024   //数据文件文件头长度
...
struct stdSCRadarObservationParam
{
	...
	struct stdSCCutParam LayerParam[30]; //各圈扫描状态设置
	...
	uint8_t  	 emonth;//		观测结束时间的月（1-12）
	uint8_t		 eday;//		观测结束时间的日（1-31）
	uint8_t		 ehour;//		观测结束时间的时（00-23）
	uint8_t		 eminute;//		观测结束时间的分（00-59）
	uint8_t		 esecond;//		观测结束时间的秒（00-59）
	uint8_t	 	 etenth;//		观测结束时间的1/100秒（00-59）
};
//基数据文件头格式，长度为1024个字节
